...
export async function getEffectiveRoles(userId: string): Promise<Role[]> {
  const assignments = await listUserRoles(userId);
  const now = new Date();
  const roles: Role[] = [];
  for (const assignment of assignments) {
    ...
    const role = await getRoleById(assignment.roleId);
...
export async function getEffectivePermissionKeys(user: User): Promise<string[]> {
  ...
  const overrides = await listUserOverrides(user.id);
  const now = new Date();
...
export async function roleSnapshotForUser(userId: string): Promise<string> {
