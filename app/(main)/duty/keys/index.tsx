...
export default function DutyKeysScreen() {
  ...
  return (
    <Screen>
      ...
      {rows.length === 0 ? (
        <EmptyState title="尚未建立鑰匙" subtitle="請由鑰匙中心先建立主檔" icon="key-outline" />
      ) : (
        rows.map((item) => (
          <ListRow
            key={item.id}
            ...
            onPress={() => router.push({ pathname: '/(main)/duty/keys/[id]', params: { id: item.id } })}
          />
        ))
      )}
    </Screen>
