...
export default function ManageImprovementsScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      void listImprovementsForActor(actor, { siteId: currentSite?.id ?? null })
        .then(setRows)
      ...
      {rows.length === 0 ? (
        ...
        rows.map((item) => (
          <ListRow
            ...
            onPress={() => router.push({ pathname: '/(main)/manage/improvements/[id]', params: { id: item.id } })}
          />
        ))
      )}
    </Screen>
