...
export default function PatrolDashboardTaskScreen() {
    ...
    <Screen>
      ...
      {task && stats ? (
        <QinCard style={{ marginBottom: spacing.md }}>
          <Text style={textStyle(colors, fontScale, 'xl', { fontWeight: '800' })}>
            {task.siteNameSnapshot} · {task.templateNameSnapshot}
          </Text>
          <Text style={textStyle(colors, fontScale, 'lg', { color: colors.accent, marginTop: 6 })}>
            {stats.completed} / {stats.totalRequired}
          </Text>
          <Text style={textStyle(colors, fontScale, 'sm', { color: colors.textMuted, marginTop: 6 })}>
            準時 {stats.onTime} · 逾時 {stats.late} · 漏巡 {stats.missed} · 異常 {stats.exceptions}
          </Text>
        </QinCard>
      ) : null}
      {points.map((point) => (
        <ListRow
          ...
          subtitle={`${point.windowLabel} · ${PATROL_POINT_LIVE_LABELS[point.liveStatus]}`}
        />
      ))}
    </Screen>
  );
